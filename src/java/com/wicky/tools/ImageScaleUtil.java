...
public class ImageScaleUtil {
    ...
    public static BufferedImage resizeUsingJavaAlgo(BufferedImage sourceImage, int width, int height) throws IOException {
        double ratio = (double) sourceImage.getWidth() / sourceImage.getHeight();
        double ratioNew = (double) width/height;
        
        if (width < 1 || ratioNew > ratio) {
            width = (int) (height * ratio + 0.4);
        } else if (height < 1 || ratioNew < ratio) {
     ...
     */
    public static void writeJpeg(BufferedImage image, File destFile, Float quality) throws IOException {
...
//        return pad(img, 4);
      ...
      public static void main(String[] args) throws IOException {
          ...
          ImageScaleUtil.writeJpeg(thumbnail, new File(filePath + fileName1 + "_thumb"+idx+++"_"+width+"x"+height+ext), 1.0f);
          ...
          thumbnail = ImageScaleUtil.resizeUsingJavaAlgo(img, width, height);
          ImageScaleUtil.writeJpeg(thumbnail, new File(filePath + fileName1 + "_thumb"+idx+++"_"+width+"x"+height+ext), 1.0f);
          
          BufferedImage crop = cropThumbnail(img, width, height);
          ImageScaleUtil.writeJpeg(crop, new File(filePath + fileName1 + "_crop1_"+width+"x"+height+ext), 1.0f);
          
          BufferedImage crop2 = cropPartThumbnail(img, width, height);
          ImageScaleUtil.writeJpeg(crop2, new File(filePath + fileName1 + "_crop2_"+width+"x"+height+ext), 1.0f);
      }
}
